  ...
  this.$get = ['$document', '$injector', '$uibModal', '$timeout', 'ImageLoader',
    function ($document, $injector, $uibModal, $timeout, ImageLoader) {
      ...
      Lightbox.openModal = function (newImages, newIndex, modalParams) {
        ...
        const templateAttribute = Lightbox.template ? 'template' : 'templateUrl';
        const templateContent = Lightbox.template ? Lightbox.template : Lightbox.templateUrl;
        const modalAttributes = {};

        modalAttributes[templateAttribute] = templateContent;
        modalAttributes.controller = ['$scope', function ($scope) {
          // $scope is the modal scope, a child of $rootScope
          $scope.Lightbox = Lightbox;
          Lightbox.keyboardNavEnabled = true;
        }];
        modalAttributes.windowClass = 'lightbox-modal';
        // store the modal instance so we can close it manually if we need to
        Lightbox.modalInstance = $uibModal.open(angular.extend(modalAttributes, modalParams || {}));
